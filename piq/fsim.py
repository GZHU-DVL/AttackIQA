...
def fsim(x: torch.Tensor, y: torch.Tensor, reduction: str = 'mean',
    ...
    Args:
        ...
        sigma_f: Ratio of the standard deviation of the Gaussian describing the logs Gabor filter's
...
def _construct_filters(x: torch.Tensor, scales: int = 4, orientations: int = 4,
    ...
    Args:
        ...
        sigma_f: Ratio of the standard deviation of the Gaussian
            describing the logs Gabor filter's transfer function
    ...
    # so that taking the logs of the radius will not cause trouble.
    ...
    # away to zero at the boundaries.  All logs Gabor filters are multiplied by
